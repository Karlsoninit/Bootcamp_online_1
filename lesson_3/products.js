...
// const getUserTShirtSize = prompt("enter your size ...").toUpperCase();

// const findSize = (props, size) => {
//   const chooseSize = [];
//   for (let product of props) {
//     const { availableSizes } = product;
//     if (availableSizes.includes(size)) {
//       chooseSize.push({ ...product, chooseSize: size });
//     }
//   }
//   return chooseSize;
// };

// const p = findSize(products, getUserTShirtSize);

// console.log(p);

const storeLogic = (props) => {
  const findBySize = () => {};
  const showDescription = () => {
    const descriptios = [];
    for (let product of props) {
      descriptios.push({ description: product.title });
    }
    return descriptios;
  };
  ...
  const showAllSize = () => {
    const uniqueSize = [];
    for (let product of props) {
      uniqueSize.push(...product.availableSizes);
    }
    return [...new Set(uniqueSize)];
  };
  return { findBySize, showDescription, showAllSize };
};

const { showAllSize, showDescription } = storeLogic(products);

console.log(showAllSize());
